...
<script>
    $(document).ready(function() {
      ...
      $('.toggle-password').change(function() {
        ...
        if (name != '' && email != '' && gender != '' && mobile != '' && password != '' && confirmPassword != '') {
            // Your logic or action when all required fields are not empty


        $.ajax({
        type:'post',
        url: '<?php echo base_url("Admin/Save_manager");?>',
        ...
        setTimeout(function(){
        window.location.href = '<?php echo base_url()?>Admin/All_Navigators';
        },2000);
